...
PALIGNEDMEM alloc_and_init(const size_t _size)
{
  ...
  std::cerr << "alloc_and_init(" << _size << ")=" << (void*)(p.get()) << std::endl;
  std::memset(p.get(), 0, _size);
  return p;
}

SCENARIO("If the system can allocate memory, no exception is thrown", "[memory]")
{
...
SCENARIO("If the system cannot allocate memory, an exception is thrown", "[memory]")
{
