...
namespace {
  struct TestFormatter {
    ...
    explicit TestFormatter(const Column::Type _type=Column::Type::unknown) : type(_type) {}
    ...
    DATETIME get_datetime(Column::PBUF) const { return boost::posix_time::from_iso_extended_string("1976-06-30T00:01:02.345"); }
    DATE get_date(Column::PBUF) const { return boost::gregorian::from_string("1976-06-30"); }
    TIME get_time(Column::PBUF) const { return boost::posix_time::duration_from_string("00:01:02.345"); }
...
SCENARIO("The Column accepts a formatter and uses correctly")
{
  GIVEN("A column with the test formatter") {
    ...
    THEN("The formatting interface is working") {
      ...
      CHECK(test.get_datetime(p) == DATETIME(DATE(1976,6,30), TIME(0,1,2)+boost::posix_time::milliseconds(345)));
      CHECK(test.get_date(p) == DATE(1976,6,30));
      CHECK(test.get_time(p) == TIME(0,1,2)+boost::posix_time::milliseconds(345));
