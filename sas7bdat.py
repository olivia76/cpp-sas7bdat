...
SAS7BDAT object
    ...
    DATE_FORMAT_STRINGS = set([
        'YYMMDD', 'MMDDYY', 'DDMMYY', 'DATE', 'JULIAN', 'MONYY', 'MONNAME'
    ])
    ...
    def _process_byte_array_with_data(self, offset, length):
        #LOG.debug(f"_process_byte_array_with_data: {offset}, {length}")
        row_elements = []
...
class Column(object):
    def __init__(self, col_id, name, label, col_format, col_type, offset, length):
        self.col_id = col_id
        self.name = name
        self.label = label
...
class RowSizeSubheader(ProcessingSubheader):
    def process_subheader(self, offset, length):
        ...
        #LOG.debug(f"RowSizeSubheader: lcs={self.properties.lcs}, lcp={self.properties.lcp}, row_length={self.properties.row_length}, row_count={self.properties.row_count}, col_count_p1={self.properties.col_count_p1 }, col_count_p2={self.properties.col_count_p2 }, mix_page_row_count={self.properties.mix_page_row_count}") 
...
class ColumnSizeSubheader(ProcessingSubheader):
    def process_subheader(self, offset, length):
        ...
        #LOG.debug(f"ColumnSizeSubheader: column_count={self.properties.column_count}")
...
class ColumnTextSubheader(ProcessingSubheader):
    def process_subheader(self, offset, length):
        ...
        #LOG.debug(f"ColumnTextSubheader: {text_block_size}, {vals[offset]}")
        ...
        if len(self.parent.column_names_strings) == 1:
            ...
            #LOG.debug(f"compression_literal={compression_literal}")
...
class ColumnNameSubheader(ProcessingSubheader):
    def process_subheader(self, offset, length):
        ...
        for i in xrange(column_name_pointers_count):
            ...
            #LOG.debug(f"ColumnNameSubheader: column_name: {idx},{col_offset},{col_len}: {name_str[col_offset:col_offset + col_len]}")
...
class ColumnAttributesSubheader(ProcessingSubheader):
    def process_subheader(self, offset, length):
        ...
        for i in xrange(column_attributes_vectors_count):
            ...
            #LOG.debug(f"ColumnAttributesSubheader: {self.parent._read_val('i', vals[col_data_offset], int_len)}, {self.parent._read_val('i', vals[col_data_len], self.COLUMN_DATA_LENGTH_LENGTH)}, {ctype}")
    ...
    def process_subheader(self, offset, length):
        ...
        self.parent.columns.append(
            Column(current_column_number,
                   ...
                   self.parent.column_data_offsets[current_column_number],
                   self.parent.column_data_lengths[current_column_number])
        )
        #LOG.debug(f"FormatAndLabelSubheader: {format_idx}, {format_start}, {format_len}: {column_format} / {label_idx}, {label_start}, {label_len}: {column_label}")
...
class DataSubheader(ProcessingSubheader):
    def process_subheader(self, offset, length):
        #LOG.debug(f"DataSubheader: {offset}, {length}")        
...
class SASHeader(object):
    ...
    def __init__(self, parent):
        ...
        if vals[self.OS_NAME_OFFSET + total_align][0]:  #[0] != 0:
    ...
    def read_page_header(self):
        ...
        #LOG.debug(f"page_type={self.parent.current_page_type},page_block_count={self.parent.current_page_block_count},page_subheaders_count={self.parent.current_page_subheaders_count}\n");
        
    def process_page_meta(self):
        self.read_page_header()
        #LOG.debug(f"process_page_meta: type={self.parent.current_page_type}")
            ...
            if pointer.compression != self.TRUNCATED_SUBHEADER_ID:
                ...
                if subheader_index is not None:
                    if subheader_index != self.DATA_SUBHEADER_INDEX:
                        ...
                        #LOG.debug(f"cls={cls}: {pointer.offset}, {pointer.length}")
                        cls(parent).process_subheader(
                            pointer.offset,
                            pointer.length
                        )
                    else:
                        #LOG.debug(f"process_page_metadata: data_subheader_pointers(pointer)")
                        parent.current_page_data_subheader_pointers.append(
                            pointer
                        )
                else:
                    #LOG.debug(f"process_page_metadata: unknown subheader signature")
    ...
    def process_subheader_pointers(self, offset, subheader_pointer_index):
        ...
        #LOG.debug(f"total_offset={total_offset}, length={length}, SubheaderPointer({subheader_offset}, {subheader_length}, {subheader_compression}, {subheader_type})");
