...
namespace cppsas7bdat {
  namespace INTERNAL {
    ...
    inline std::string_view get_string(const uint8_t* _buf, const size_t _length) noexcept {
      return std::string_view(reinterpret_cast<const char*>(_buf), _length);
    }

    inline std::string_view get_string_trim_0(const uint8_t* _buf, size_t _length) noexcept {
      while(_length && (_buf[_length-1] == 0 || std::isspace(_buf[_length-1]))) { --_length; }
      return get_string(_buf, _length);
    }

    inline std::string_view get_string_trim(const uint8_t* _buf, size_t _length) noexcept {
      size_t offset = 0;
    ...
    struct _GET_VAL {
      ...
      static inline _Tp get(const uint8_t* _buf) noexcept
      {
	_Tp x;
    ...
    struct ENDIAN {
      ...
      static inline _Tp get_val(const uint8_t* _buf) noexcept
      {
	//const _Tp* x = reinterpret_cast<const _Tp*>(_buf);  // UB: https://en.cppreference.com/w/cpp/language/reinterpret_cast
	_Tp x;
	std::memcpy(&x, _buf, sizeof(_Tp));
