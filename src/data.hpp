...
namespace cppsas7bdat {
  namespace INTERNAL {
	...
	bool next() const noexcept final { return current_row_on_page == data_subheaders.size(); }
	...
	data_type(const size_t _block_count, const size_t _offset, const size_t _row_length)
	  : block_count(_block_count),
	    offset(_offset),
	    row_length(_row_length)
	{
	  D(spdlog::info("PAGE::data_type: {}, {}, {}\n", block_count, offset, row_length));
	}
	
	bool next() const noexcept final { return current_row_on_page == block_count; }
	OFFSET_LENGTH read_line() noexcept final {
	  return std::make_pair(offset + row_length * current_row_on_page, row_length);
	}
      };
	...
	: READ_PAGE<_DataSource, _endian, _format>(std::move((READ_PAGE<_DataSource, _endian, _format>&&)_rm)),
	...
	if(current_page_header.type == PAGE_META_TYPE) {
	  ...
	  page.reset(new PAGE::data_type(current_page_header.block_count, page_bit_offset + 8, metadata->row_length));
	  return true;
	...
	while(true) {
	  ...
	  if(build_page()) return true;
	}
      }

      void process_page_metadata(DATASUBHEADERS& _data_subheaders) {
	auto psh = [&](const PAGE_SUBHEADER& _subheader) {
		     if(DataSubHeader::check(metadata, _subheader)) process_DATA_SUBHEADER(_subheader, _data_subheaders);
		   };
	process_page_subheaders(psh);
      }

      void process_DATA_SUBHEADER(const PAGE_SUBHEADER& _subheader, DATASUBHEADERS& _data_subheaders)
      {
