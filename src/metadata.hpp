...
namespace cppsas7bdat {
  namespace INTERNAL {
    ...
    constexpr const std::string_view COLUMN_TIME_FORMAT[] = {
					   "TIME"
    };
    ...
    struct METADATA_CONSTANT;

    template<>
    struct METADATA_CONSTANT<Format::bit64> {
      ...
      static constexpr BYTE ROW_SIZE_SUBHEADER          [][8] = { { 0x00,0x00,0x00,0x00, 0xF7,0xF7,0xF7,0xF7 }, { 0xF7,0xF7,0xF7,0xF7, 0x00,0x00,0x00,0x00 } };
      static constexpr BYTE COLUMN_SIZE_SUBHEADER       [][8] = { { 0x00,0x00,0x00,0x00, 0xF6,0xF6,0xF6,0xF6 }, { 0xF6,0xF6,0xF6,0xF6, 0x00,0x00,0x00,0x00 } };
      static constexpr BYTE SUBHEADER_COUNTS_SUBHEADER  [][8] = { { 0x00,0xFC,0xFF,0xFF, 0xFF,0xFF,0xFF,0xFF }, { 0xFF,0xFF,0xFF,0xFF, 0xFF,0xFF,0xFC,0x00 } };
      static constexpr BYTE COLUMN_TEXT_SUBHEADER       [][8] = { { 0xFD,0xFF,0xFF,0xFF, 0xFF,0xFF,0xFF,0xFF }, { 0xFF,0xFF,0xFF,0xFF, 0xFF,0xFF,0xFF,0xFD } };
      static constexpr BYTE COLUMN_NAME_SUBHEADER       [][8] = { { 0xFF,0xFF,0xFF,0xFF, 0xFF,0xFF,0xFF,0xFF } };
      static constexpr BYTE COLUMN_ATTRIBUTES_SUBHEADER [][8] = { { 0xFC,0xFF,0xFF,0xFF, 0xFF,0xFF,0xFF,0xFF }, { 0xFF,0xFF,0xFF,0xFF, 0xFF,0xFF,0xFF,0xFC } };
      static constexpr BYTE FORMAT_AND_LABEL_SUBHEADER  [][8] = { { 0xFE,0xFB,0xFF,0xFF, 0xFF,0xFF,0xFF,0xFF }, { 0xFF,0xFF,0xFF,0xFF, 0xFF,0xFF,0xFB,0xFE } };
      static constexpr BYTE COLUMN_LIST_SUBHEADER       [][8] = { { 0xFE,0xFF,0xFF,0xFF, 0xFF,0xFF,0xFF,0xFF }, { 0xFF,0xFF,0xFF,0xFF, 0xFF,0xFF,0xFF,0xFE } };
    };

    template<>
    struct METADATA_CONSTANT<Format::bit32> {
      ...
      static constexpr BYTE SUBHEADER_COUNTS_SUBHEADER  [][4] = { { 0x00,0xFC,0xFF,0xFF }, { 0xFF,0xFF,0xFC,0x00 } };
      static constexpr BYTE COLUMN_TEXT_SUBHEADER       [][4] = { { 0xFD,0xFF,0xFF,0xFF }, { 0xFF,0xFF,0xFF,0xFD } };	
      static constexpr BYTE COLUMN_NAME_SUBHEADER       [][4] = { { 0xFF,0xFF,0xFF,0xFF } };
      static constexpr BYTE COLUMN_ATTRIBUTES_SUBHEADER [][4] = { { 0xFC,0xFF,0xFF,0xFF }, { 0xFF,0xFF,0xFF,0xFC } };
      static constexpr BYTE FORMAT_AND_LABEL_SUBHEADER  [][4] = { { 0xFE,0xFB,0xFF,0xFF }, { 0xFF,0xFF,0xFB,0xFE } };
	...
	D(fmt::print(stderr, "process_page: type={}\n", current_page_header.type));
	...
	auto psh = [&](const PAGE_SUBHEADER& _psh) {
		     if(process_subheader(_psh, _metadata)) data_reached = true;
		   };
	...
	if(match_signature(COLUMN_TEXT_SUBHEADER      , subheader_signature)) { process_COLUMN_TEXT_SUBHEADER      (_subheader, _metadata); return false; }
	if(match_signature(COLUMN_NAME_SUBHEADER      , subheader_signature)) { process_COLUMN_NAME_SUBHEADER      (_subheader, _metadata); return false; }
	if(match_signature(COLUMN_ATTRIBUTES_SUBHEADER, subheader_signature)) { process_COLUMN_ATTRIBUTES_SUBHEADER(_subheader, _metadata); return false; }
	if(match_signature(FORMAT_AND_LABEL_SUBHEADER , subheader_signature)) { process_FORMAT_AND_LABEL_SUBHEADER (_subheader, _metadata); return false; }
	if(match_signature(COLUMN_LIST_SUBHEADER      , subheader_signature)) { process_COLUMN_LIST_SUBHEADER      (_subheader, _metadata); return false; }

	if(DataSubHeader::check(_metadata, _subheader)) { process_DATA_SUBHEADER(_subheader); return true; }

	// Unknown subheader...
	fmt::print(stderr, "Unknown subheader signature!");
	return false;
      }	

      template<size_t n, size_t m>
      static bool match_signature(const BYTE (&_s)[n][m], const BYTES _signature) noexcept
      {
	return std::any_of(std::begin(_s), std::end(_s), [sig=_signature.data()](auto s) { return std::memcmp(s, sig, m) == 0; });
      }

      void process_ROW_SIZE_SUBHEADER([[maybe_unused]] const PAGE_SUBHEADER& _subheader,
				      Properties::Metadata* _metadata) const noexcept
      {
	D(fmt::print(stderr, "READ_METADATA::process_ROW_SIZE_SUBHEADER: {}, {} : ", _subheader.offset, _subheader.length));
	...
	_metadata->row_count = buf.get_uinteger(_subheader.offset + 6*integer_size);
	...
	D(fmt::print(stderr, "lcs={}, lcp={}, row_length={}, row_count={}, col_count_p1={}, col_count_p2={}, mix_page_row_count={}\n",
      ...
      void process_COLUMN_SIZE_SUBHEADER([[maybe_unused]] const PAGE_SUBHEADER& _subheader,
					 Properties::Metadata* _metadata) const noexcept
      {
	D(fmt::print(stderr, "READ_METADATA::process_COLUMN_SIZE_SUBHEADER: {}, {} : ", _subheader.offset, _subheader.length));
	_metadata->column_count = buf.get_uinteger(_subheader.offset + integer_size);
	D(fmt::print(stderr, "column_count={}\n", _metadata->column_count));
	if(_metadata->col_count_p1 + _metadata->col_count_p2 != _metadata->column_count)
	  fmt::print(stderr, "Column count mismatch\n");
      }
	...
	if(column_texts.size()) {
	  ...
	  if(_length && std::isprint(ct[_offset])) {
	    while(_length && std::isspace(ct[_offset])) { ++_offset; --_length; }
	    while(_length && (!std::isprint(ct[_offset+_length-1]) || std::isspace(ct[_offset+_length-1]))) { --_length; }
	...
	return {};
      ...
      {
	D(fmt::print(stderr, "READ_METADATA::process_COLUMN_NAME_SUBHEADER: {}, {}\n", _subheader.offset, _subheader.length));
	...
	for(size_t offset = _subheader.offset + integer_size + 8; offset <= offset_max; offset += integer_size+8) {
      ...
      void process_FORMAT_AND_LABEL_SUBHEADER([[maybe_unused]] const PAGE_SUBHEADER& _subheader,
					      [[maybe_unused]] Properties::Metadata* _metadata) noexcept
      {
	D(fmt::print(stderr, "READ_METADATA::process_FORMAT_AND_LABEL_SUBHEADER: {}, {}\n", _subheader.offset, _subheader.length));
	...
	D(fmt::print(stderr, "column format: {}, {}, {}: ", format_idx, format_offset, format_length));
	const auto column_format = get_column_text_substr(format_idx, format_offset, format_length);
	D(fmt::print(stderr, "{}\n", column_format));
	D(fmt::print(stderr, "column_label: {}, {}, {}: ", label_idx, label_offset, label_length));
	...
	column_labels.emplace_back(column_label);
      ...
      void process_COLUMN_LIST_SUBHEADER([[maybe_unused]] const PAGE_SUBHEADER& _subheader,
					 [[maybe_unused]] Properties::Metadata* _metadata) const noexcept
      {
	D(fmt::print(stderr, "READ_METADATA::process_COLUMN_LIST_SUBHEADER: {}, {}\n", _subheader.offset, _subheader.length));
      }

      void process_DATA_SUBHEADER([[maybe_unused]] const PAGE_SUBHEADER& _subheader) noexcept
      {
	D(fmt::print(stderr, "READ_METADATA::process_DATA_SUBHEADER: {}, {}\n", _subheader.offset, _subheader.length));
	...
	auto get_value = [&](auto arg, auto vals, const size_t icol) {
			   ...
			   if(n != ncols) fmt::print(stderr, "Mismatch between the expected number of columns ({}) and the number ({}) of '{}' values!\n", ncols, vals.size(), arg);
			   return vals.at(icol);
			 };
	...
	for(size_t icol=0; icol<ncols; ++icol) {
      ...
      template<size_t n>
      static bool match_column_format(const std::string_view (&_list)[n], const std::string_view& _f) noexcept
      {
	return std::any_of(std::begin(_list), std::end(_list), [_f](auto _g) { return _f == _g; });
      }
