...
namespace cppsas7bdat {
  namespace INTERNAL {
    ...
    struct DataSubHeader {
      ...
      DataSubHeader(const PAGE_SUBHEADER& _subheader) : offset(_subheader.offset), length(_subheader.length) {}

      // Is it a data subheader?
      static bool check(const Properties::Metadata* _metadata, const PAGE_SUBHEADER& _subheader) noexcept
      {
	return ( (_metadata->compression != Compression::none) &&
		 (_subheader.compression == COMPRESSED_SUBHEADER_ID || _subheader.compression == 0) &&
