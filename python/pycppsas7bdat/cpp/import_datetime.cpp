...
namespace {
  ...
  {
    static const long resolution = boost::posix_time::time_duration::ticks_per_second();
    const long fracsecs = d.fractional_seconds();
    if(resolution > 1000000) return fracsecs / (resolution / 1000000);
    else return fracsecs * (1000000 / resolution);
  }
  ...
  struct ptime_to_python_datetime
  {
    ...
    {
      if(pt.is_not_a_date_time()) return Py_None;      
  ...
  struct tduration_to_python_delta
  {
    ...
    {
      if(d.is_not_a_date_time()) return Py_None; 
      long hours = d.hours();
      long days = hours / 24;
      if(days == 0) {
	...
	long usecs = get_usecs(d);
	if(days < 0) usecs = 1000000-1 - usecs;
  ...
  struct tduration_from_python_delta
  {
    ...
    static void* convertible(PyObject * obj_ptr)
    {
      if(!PyDelta_Check(obj_ptr)) return nullptr;
      return obj_ptr;
    }
			  ...
			  boost::python::converter::rvalue_from_python_stage1_data* data)
    {
      ...
      if (is_negative) duration = duration.invert_sign();
      
      void* storage = ( (boost::python::converter::rvalue_from_python_storage<boost::posix_time::time_duration>*)data )->storage.bytes;
  ...
  struct gregorian_date_to_python_date
  {
    ...
    {
       if(date.is_not_a_date()) return Py_None; 
  ...
  struct gregorian_date_from_python_date
  {
			  ...
			  boost::python::converter::rvalue_from_python_stage1_data* data)
    {
      ...
      // Create posix time object
      void* storage = ( (boost::python::converter::rvalue_from_python_storage<boost::gregorian::date>*)data )->storage.bytes;
