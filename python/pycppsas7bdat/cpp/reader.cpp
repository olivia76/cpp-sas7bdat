...
namespace pycppsas7bdat {
  struct Reader : public cppsas7bdat::Reader {
    Reader(const char* _filename, PyObject* _sink, PyObject* _include, PyObject* _exclude)
      : cppsas7bdat::Reader(build(_filename, _sink, _include, _exclude))
    {
    }

    static cppsas7bdat::ColumnFilter::IncludeExclude filter(PyObject* _include, PyObject* _exclude)
    {
      auto list_to_set = [](std::string _context, auto& _set, PyObject* _object) -> void {
			   if(!_object || _object == Py_None) return;
			   if(!PyList_Check(_object)) {
			     throw std::runtime_error(fmt::format("{} argument is not a list", _context));
			   }
			   ...
			   for (int i = 0; i<n; ++i) {
			     ...
			     try {
			       ...
			       throw std::runtime_error(fmt::format("Element of {} argument is not a string", _context));
			     }
			   }
			 };
    ...
    static cppsas7bdat::Reader build(const char* _filename, PyObject* _sink, PyObject* _include, PyObject* _exclude)
    {
  ...
  boost::shared_ptr<Reader> create_reader(std::string _filename, PyObject* _sink, PyObject* _include=nullptr, PyObject* _exclude=nullptr)
  {
    return boost::shared_ptr<Reader>(new Reader(_filename.c_str(), _sink, _include, _exclude));
  }
  ...
  void bind_reader()
  {
    ...
    class_< Reader, boost::noncopyable >("Reader", no_init)
      .def("__init__", make_constructor(&create_reader,
